...
export class GraphViewServer extends EventEmitter {
	...
	private getViewSettings(): GraphViewSettings {
		return (
			vscode.workspace
				.getConfiguration()
				.get<GraphViewSettings>("cosmosDB.graph.viewSettings") ||
			<GraphViewSettings>{}
		);
	}
					...
					if (results.limitedVertices.length) {
						try {
							...
							throw new EdgeQueryError(
								`Error querying for edges: ${edgesError.message || edgesError}`,
							);
						}
					}
				},
		...
		while (true) {
			...
			try {
				...
				if (this.isErrorRetryable(err)) {
					if (iTry >= maxRetries) {
						this.log(
							`Max retries reached for query ${queryId}: ${truncateQuery(gremlinQuery)}`,
						);
					} else {
						continue;
					}
	...
	private async _executeQueryCore(
		...
		if (this.configuration.gremlinEndpoint) {
			...
			if (firstValidError) {
				...
				throw new Error(
					`Could not find a valid gremlin endpoint for ${this.configuration.graphName}.\r\n\r\nTried ${this.configuration.possibleGremlinEndpoints.map((e) => e.host).join(", ")}`,
				);
			}
		}
	}
	...
	private async _executeQueryCoreForEndpoint(
		...
		this.log(
			`Executing query #${queryId} (${endpoint.host}:${endpoint.port}): ${truncateQuery(gremlinQuery)}`,
		);
	...
	private isErrorRetryable(err: { message?: string }) {
		...
		if (err.message) {
			if (
				err.message.match(/Status *: *429/) ||
				err.message.match(/RequestRateTooLarge/)
			) {
